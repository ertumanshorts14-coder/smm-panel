    ...
    <div className="min-h-screen flex flex-col bg-[#0A0E27] text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-24 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative hidden md:block">
            <div className="absolute -top-8 -left-8 w-14 h-14 bg-white rounded-2xl shadow-2xl flex items-center justify-center text-2xl rotate-[-10deg] z-10 animate-float">
              🎵
            </div>
            <div
              className="absolute -top-12 right-8 w-12 h-12 bg-gradient-to-br from-pink-500 to-purple-500 rounded-2xl shadow-2xl flex items-center justify-center text-white font-bold rotate-[15deg] z-10 animate-float"
              style={{ animationDelay: '0.5s' }}
            >
              📷
            </div>
            <div
              className="absolute -bottom-6 -left-6 w-12 h-12 bg-blue-500 rounded-2xl shadow-2xl flex items-center justify-center text-white font-bold -rotate-[15deg] z-10 animate-float"
              style={{ animationDelay: '1s' }}
            >
              f
            </div>
            <div
              className="absolute -bottom-4 right-0 w-12 h-12 bg-black rounded-2xl shadow-2xl flex items-center justify-center text-white font-bold rotate-[10deg] z-10 animate-float"
            ...
            <div className="relative bg-gradient-to-br from-slate-900 to-slate-950 border border-white/10 rounded-2xl p-4 shadow-2xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="ml-auto text-xs text-slate-400 font-medium">SMUQ SMM Panel</div>
              </div>
              <div className="bg-[#0A0E27] rounded-lg p-4 space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-3">
                    <p className="text-[10px] text-slate-500">Balance</p>
                    <p className="text-sm font-bold text-white">Rs 1,25,000</p>
                  </div>
      ...
      <section className="border-y border-white/5 bg-white/[0.02] backdrop-blur">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-12 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          <FadeIn delay={0}>
            <StatCard icon="👥" value="10,000+" label="Happy Users" />
          </FadeIn>
          <FadeIn delay={0.1}>
            <StatCard icon="📦" value="1,50,000+" label="Orders Completed" />
          </FadeIn>
          <FadeIn delay={0.2}>
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <FadeIn delay={0}>
              <FeatureCard icon="⚡" title="Instant Delivery" description="Lightning fast delivery for all services. Get results in minutes, not hours." />
            </FadeIn>
            <FadeIn delay={0.1}>
              <FeatureCard icon="💎" title="High Quality Services" description="Premium quality services from trusted providers. 100% real and safe." />
            </FadeIn>
            <FadeIn delay={0.2}>
              <FeatureCard icon="💰" title="Affordable Prices" description="Competitive prices with the best value for money. Save more, grow faster." />
            </FadeIn>
            <FadeIn delay={0.3}>
              <FeatureCard icon="📊" title="Real Results" description="Real engagement from real accounts. No bots, no fake data, just results." />
            </FadeIn>
            <FadeIn delay={0.4}>
              <FeatureCard icon="🛡️" title="Secure & Safe" description="Your account security is our top priority. 100% safe and secure." />
            </FadeIn>
            <FadeIn delay={0.5}>
              <FeatureCard icon="🎧" title="24/7 Support" description="Round the clock support via WhatsApp, live chat, and email." />
            </FadeIn>
          </div>
        </div>
      </section>
...
function StatCard({ icon, value, label }: { icon: string; value: string; label: string }) {
  return (
    <div className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-4 md:p-5 text-center hover:bg-white/10 transition h-full">
...
function FeatureCard({ icon, title, description }: { icon: string; title: string; description: string }) {
  return (
    <div className="bg-white/[0.03] backdrop-blur border border-white/10 rounded-2xl p-5 md:p-6 hover:bg-white/[0.06] hover:border-purple-500/30 transition group h-full">
